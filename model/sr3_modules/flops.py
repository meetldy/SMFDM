...
def count_ops_and_params(model, example_inputs, layer_wise=False):
    ...
    # Adjusted logic to correctly handle dictionary input
    simplified_inputs = {
        'inp': example_inputs['inp'],
        'coord': torch.zeros_like(example_inputs['coord']) if 'coord' in example_inputs else None,  # 或其他适当的默认值
        'cell': torch.zeros_like(example_inputs['cell']) if 'cell' in example_inputs else None,
        'gt': torch.zeros_like(example_inputs['gt']) if 'gt' in example_inputs else None,
        'scaler': torch.zeros_like(example_inputs['scaler']) if 'scaler' in example_inputs else None,
    }

    if isinstance(example_inputs, dict):
        _ = flops_model(simplified_inputs)  # 使用简化后的输入
    elif isinstance(example_inputs, (tuple, list)):  # Now tuple and list come second
